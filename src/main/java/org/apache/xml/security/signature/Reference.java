 ...
 * 
 * Changes:
 *   * modified to allow to use ResourceResolver that access to private data. This data cannot be managed by Reference, only its digest.
 *     XmlSigntaureInput doesn't return data, but digest.
 * @author David Santos
 */
...
private Element digestValueElement;
    ...
    * @throws ReferenceNotInitializedException
    */
   protected byte[] getDigestPrivateData() throws ReferenceNotInitializedException {
	   try {
		   ResourceResolver resolver = getResourceResolver();
		   Attr URIAttr = this._constructionElement.getAttributeNodeNS(null,
				   Constants._ATT_URI);

		   // send hash algorithm name to resolver
		   if (!resolver.understandsProperty(Constants.PROPERTY_RR_ALG_DIGEST))
			   throw new ReferenceNotInitializedException("empty");
		   ...
		   resolver.setProperty("digest.algorithm", uri);
   ...
   /**
    * Returns the ResourceResolver which is used to de-referencing de URI attribute
    * @return the ResourceResolver of this reference
    * @throws ReferenceNotInitializedException If the resolver found any
    *  problem resolving the reference
    */
   private ResourceResolver getResourceResolver() throws ReferenceNotInitializedException {
	   try {
		   Attr URIAttr = this._constructionElement.getAttributeNodeNS(null,
				   Constants._ATT_URI);
   ...
   private byte[] calculateDigest(boolean validating)
 	  ...
 	  ResourceResolver rr = this.getResourceResolver();
	  if (rr.canAccessData()) {
	      try {
			 ...
			 if (this.useC14N11 && !validating &&
			     ...
			     if (transforms == null) {
		             ...
		             this._constructionElement.insertBefore(transforms.getElement(), digestMethodElem);
			     }
		  ...
		  return this.getDigestPrivateData();
	  }
   }
